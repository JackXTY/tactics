Shader "Universal Render Pipeline/Post Effect/PostRaindrop"
{
    ...
    Properties
    {
        ...
        [Toggle(_FOG_SCREEN)]_FogScreen("Fog Screen", Float) = 0
    }
    SubShader
    {
        ...
        HLSLINCLUDE
            ...
            #pragma multi_compile_local _ _FOG_SCREEN
            ...
            half2 rainEffect(float2 iuv, out float fogTrail)
            {
                ...
                // simulate trace of raindrop falling in screen 
                half w = iuv.y * 4;
                ...
                // draw a circle, interpolate length(uv) with smoothstep(A, B, len), [A, B] => [0, 1]

                // add some small raindrop along the trail
...
#ifdef _FOG_SCREEN
                // let the trace wipe the fog, then the fog would come back
                half4 col = length(offset) > 0.001f ? 
                    SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv) : 
                    lerp(
                        SAMPLE_TEXTURE2D(_BlurTex, sampler_BlurTex, uv),
                        SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv),
                        fogTrailSum);

#else
                // let trace add some fog
                half4 col = lerp(
                    SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv),
                    SAMPLE_TEXTURE2D(_BlurTex, sampler_BlurTex, uv),
                    fogTrailSum);
#endif
                // TODO: Transition between fogTrail & blur
                // might need to handle two different cases (has blur/no blur)
                // Combine with depth of field, use same blur texture?
                /*
                if (blur > 0.5) {
                    half del = 0.002;
